...
export default class BlockFurnaceTrait extends BlockTrait {
  public static readonly identifier = "furnace";
  public static readonly types = [BlockIdentifier.Furnace, BlockIdentifier.LitFurnace];

  public static MAX_COOKTIME = 200;
  public static MAX_BURNDURATION = 1600;
  public static MAX_BURNTIME = 1600;

  private readonly container = new BlockContainer(this.block, ContainerType.Furnace, 3);
  ...
  public onTick(_details: TraitOnTickDetails): void {
    ...
    const smeltable = ingredient ? smeltableTypes.find((entry) => entry.input === ingredient.identifier) : undefined;
    ...
    if (this.cookTime >= BlockFurnaceTrait.MAX_COOKTIME) {
      this.cookTime = 0;
      this.dirty = true;
  ...
  private setLit(lit: boolean): void {
    if (!lit && this.block.identifier === BlockIdentifier.Furnace) return;

    const permutation = lit
      ? this.dimension.world.blockPalette.resolvePermutation(BlockIdentifier.LitFurnace, this.block.permutation.state as any)
      : this.dimension.world.blockPalette.resolvePermutation(BlockIdentifier.Furnace, this.block.permutation.state as any);
